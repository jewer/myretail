...
class ProductServicesSpec extends FlatSpec with Matchers with ScalatestRouteTest with ProductServices {
  ...
  override lazy val nameServiceFlow = Flow[HttpRequest].map { request =>
    HttpResponse(status = BadRequest, entity = marshal("Bad Request"))
  }

  /*"ProductServices" should "return a product" in {
    Get(s"/name/1234") ~> routes ~> check {
      status shouldBe OK
      contentType shouldBe `application/json`
      responseAs[Product] shouldBe Product(1234, "test1", Price(100, "USD"))
    }
  }

  it should "respond with bad request on incorrect product key" in {
    Get("/product/asdfg") ~> routes ~> check {
